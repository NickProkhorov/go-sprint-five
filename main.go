...
func (t Training) distance() float64 {
	...
	return float64(t.Action) * t.LenStep / float64(MInKm) // дистанция в км
}
...
func (t Training) meanSpeed() float64 {
	...
	return t.distance() / t.Duration.Hours() // скорость в км/ч
}
...
type Walking struct {
	// добавьте необходимые поля в структуру
	Training
	Height float32 // рост пользователя
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	wlkAvgSpd := w.Training.meanSpeed()
	return ((CaloriesWeightMultiplier*w.Training.Weight + (math.Pow((wlkAvgSpd*KmHInMsec), 2)/(float64(w.Height)/float64(CmInM)))*CaloriesSpeedHeightMultiplier*w.Training.Weight) * float64(w.Training.Duration.Hours()) * MinInHours)
}
...
func (s Swimming) Calories() float64 {
	...
	return (meanSpeed + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Training.Weight * s.Training.Duration.Hours()
}
